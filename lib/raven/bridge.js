  ...
  var Bridge = function(raven, dbData) {
    ...
    function addPlayer(socket, user, role) {
      ...
      handleMessage('placeGuerrilla', function(data) {
        var result = guerrilla_checkers.placeGuerrillaPiece(data.position);
        if (!guerrilla_checkers.getWinner() && guerrilla_checkers.isSoldierTurn()) {
          var state = {};
          state[GUERRILLA_ROLE] = "PEND";
          state[COIN_ROLE] = "ATTN";
      ...
      handleMessage('moveCOIN', function(data) {
        var result = guerrilla_checkers.moveSoldierPiece(data.piece, data.position);
        if (!guerrilla_checkers.getWinner() && guerrilla_checkers.isGuerrillaTurn()) {
          var state = {};
