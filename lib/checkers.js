...
function GameState() {
  ...
  /**
   * Retrieves the guerrilla piece at the specified position.
   * @param position The zero-based position to check for a piece
   * @return {GuerrillaPiece} The piece at the specified position,
   *         or null if not found.
   */
  ...
  var removePiece = function(arrPieces, piece) {
    if (piece) {
      for(var idx = 0; idx < arrPieces.length; ++idx) {
...
GameState.prototype.advancePhase = function() {
  ...
  if (this.arrPhases[this.currentPhase] !== this.arrPhases[old]) {
    this.turnCount += 1;
  }
...
GameState.prototype.getPotentialSoldierMoves = function(soldierPiece) {
  ...
  for(var idx = 0; idx < testPositions.length; ++idx) {
...
GameState.prototype.getPotentialGuerrillaMoves = function() {
  ...
  if (this.getGuerrillaPieces().length === 0) {
    ...
    for (i = 0; i < testPositions.length; i++) {
      position = testPositions[i];
      if (this.isValidGuerrillaPlacement(position)) {
        moves.push(position);
      }
    }
  } else {
    ...
    for (i = 0; i < this.getGuerrillaPieces().length; i++) {
      ...
      for (j = 0; j < testPositions.length; j++) {
        ...
        if (this.isValidGuerrillaPlacement(piece)) {
...
GameState.prototype.placeGuerrillaPiece = function(piecePosition) {
  ...
  for(var idx = 0; idx < arrPotentialCaptures.length; ++idx) {
...
GameState.prototype.forfeit = function(role) {
...
GameState.prototype.getWinner = function() {
  ...
  if (me.arrSoldierPieces.length === 0) {
    return GUERRILLA_ROLE;
  } else if (me.remainingGuerrillaPieces <= 0 ||
...
GameState.prototype.getScores = function() {
  ...
  scores[winner] = this.remainingGuerrillaPieces;
